...
Thread::Yield ()
{
    ...
    //  SJF
    DEBUG(dbgMLFQ, "[UpdateRemainingBurstTime] Tick[" << kernel->stats->totalTicks << "]: Thread [" << nextThread->getID() << "] update remaining burst time" << this->getID() << " is replaced, and it has executed " << "ticks");
    // ??
    int new_burst = 0.5*this->getRunTime() + 0.5*this->getRemaningWaitTime();
    this->setRemaningWaitTime(new_burst);
...
Thread::Sleep (bool finishing)
{
    ...
    int new_RemainingBurstTime = 0.5*this->getRunTime() + 0.5*this->getRemaningWaitTime();
    this->setRemaningWaitTime(new_RemainingBurstTime);
    // ?
    this->setRunTime(0);
    kernel->scheduler->Run(nextThread, finishing);
    //<TODO>
